...
- **Monthly digest mislabeled ~a month of blocks as "this week"** — `WeeklyDigest` switches to monthly cadence after week 8 (firing only on the first Sunday of each month), but `showDigest()` unconditionally used the weekly string. Because the weekly alarm keeps firing in monthly mode and most firings return early *without* resetting the counter, the number shown by the time the monthly digest actually fires is ~4-5 weeks of accumulated blocks — labeled "this week," overstating weekly volume roughly 4x to the elderly audience the digest is meant to keep calibrated. Added `digest_text_monthly` ("...this month.") to all four locales and threaded an `isMonthly` flag into `showDigest()`.
- **`PostCallAdvisor` duplicate warning for the same call** — a police/tax-agency/high-risk-hour heads-up warning could fire while the phone rings, then `PostCallAdvisor` would fire a second, separately-worded #9110 advisory for the exact same call if answered >30s. Added `WarningNotifier.recordWarningShown()`/`wasWarnedRecently()` (10-minute cross-channel dedup window); `PostCallAdvisor.maybeShow()` now skips its own notification when a heads-up warning already covered the call. Deliberately narrow — `TrustNotifier` and `WeeklyDigest` are untouched, not a general N-channel suppression framework.
- **`docs/play_data_safety.json` over-declared `READ_CALL_LOG`** — listed with the justification "required by the CallScreeningService contract," but `CallScreeningService` gets call info via `Call.Details` once the system grants `ROLE_CALL_SCREENING`; the manifest never declared this permission. Removed; the file's permission list now matches the manifest's actual two permissions exactly. Also refreshed the file's stale architecture inventory (a "15-point decision engine" description when `decide()` is now Layer 16; missing `TaxAgencyDirectory`/`ManualBlock`/`AllowSuffixStore`; stale offline-database entry counts) and replaced a `privacy_policy_url` template placeholder domain that was never a real, specified address with an explicit "NOT YET SET" marker.
- **zh/ko locales missing 21 keys** — `values-zh/strings.xml` and `values-ko/strings.xml` had only 65 of the 86 (now 87) keys present in `values`/`values-ja`, so Chinese/Korean users saw untranslated English fallback text in the manual-block Settings UI, the allowed-numbers list, the tax-agency impersonation warning, and the urgent outbound-callback warning. Translated all missing keys; all four locale files now have an identical key set.
- **No launcher icon fallback below API 26** — `minSdk=24`, but the only launcher icon resource was the adaptive icon (`mipmap-anydpi-v26`, API 26+). A pre-existing `res/drawable/ic_launcher.xml` claimed to be the pre-O fallback, but that claim was itself wrong: `mipmap` and `drawable` are distinct Android resource types, so a `drawable` resource can never satisfy a `@mipmap/` manifest reference. Generated real PNG icons at all five density buckets reproducing the adaptive icon's design (solid `#FF8C42` background, white circle at the same 22/108 radius ratio).

### Changed
- **`ScamPrefixSeed` KDoc clarified, not behavior-changed** — the source-rationale comment listed "+1 (北米経由)" alongside +675/+7/+44, all of which *are* in `elevatedRiskCountryCodes`, while +1 itself deliberately is not (it is also the highest-volume *legitimate* international corridor to JP; an unsolicited +1 call is still silenced by the generic foreign-unsolicited rule, just labeled `FOREIGN_GENERIC` rather than `FOREIGN_ELEVATED`). Rewrote the KDoc to state the exclusion explicitly and refreshed the source rationale with 2026-07 NPA/警視庁 findings. Added a regression test (`not_elevated_us_plus1`) so a future edit can't silently promote +1.
- **`SpamCache` KDoc corrected from "LRU" to "FIFO"** — the header claimed LRU eviction; `add()` returns early when a hash is already present and never refreshes its position in the eviction order (its own inline comment already said FIFO correctly). Corrected the header; noted why the distinction is inconsequential at `MAX_ENTRIES = 10,000`.
- **Three stale UI-layer class-doc claims corrected** (comments only, no behavior change): `HistoryActivity` described Allow as an allowlist of 5 `BlockReason`s when the actual `canAllow` is a denylist of 2; `SettingsActivity` said "the only user-configurable setting is the 3-slot family number list," stale from before Manual Block and Allowed Numbers were added to the same screen; `OrangeWidget` said "Tapping does nothing" while the code has an explicit tap target (opens History or re-onboarding).
- Documented (not changed) that the STIR/SHAKEN carrier-verification gate (`decide()` Layer 10) is dormant on Japanese carriers as of 2026-07 (総務省 still at the discussion stage) — kept as zero-cost forward-insurance that activates automatically for any network that does attest.

### Added
- **`PauseTileTest`** — `PauseTile.isPaused()` is the single source of truth every "Pause means every call rings" call site reads (including the `RepeatCallerTracker` fix above), yet had zero direct test coverage. Adds 7 cases: never-set, near-future/expired/exactly-now `pausedUntil`, and both sides of the `MAX_PAUSE_MS` (2h) backward-clock-jump guard.
- **`RESEARCH_BASIS.md` 2026 field-data section** — logs three 2026-07 findings that validate (without changing) the engine's design: NPA reclassified ニセ警察詐欺 as its own top-level 手口 (~70% of loss); ~75.5% of fraud-associated numbers are international; 警視庁 attributes a 38.8% YoY drop in Tokyo cases partly to a doubling of anti-fraud app installs. Also logs the CHI 2025 finding (arXiv:2502.03964) that in-call real-time warnings disrupt scammers' psychological pressure, as the first evidence behind the (still user-judgment-gated) full-screen-intent candidate in `FEATURE_AUDIT.md`.
