...
| W2 | `toDomestic()` が bare NSN を棄権 → **テスト2件が意図的に失敗**(可視シグナル)| §1-7。**ITU-T E.164 により棄権が契約上正しいと確定**(先頭0はトランクプレフィックスで加入者番号の一部でない)。**推奨はテスト側の修正**だが、意図的シグナルを消す変更のため人間の承認待ち |
...
| W6 | Allow(末尾4桁)と Restore(完全番号)の解除二重化 | §2-2。**判断材料は精密化済み**(ハッシュ保存の増分は小さいが「display-only」の主張強度が下がる/現状は1万分の1を16層迂回で許可)。**残る判断は「主張を弱めてよいか」のみ** |
| W7 | STIR/SHAKEN 層は日本キャリアでは休眠(国内未導入)| 仕様として文書化済み。触らない |
| W8 | 警告通知の実効性が未検証(オフラインゆえテレメトリ不可)| **文言は改善済み**(IEEE S&P 2025 の contextual warning 知見で `police_warn_body` を4ロケール改訂)。**残るのはフルスクリーン化=攻撃性を上げるか**の UX 判断のみ(§1-4)|
...
2026-07 の監査で **W1/W3/W4/W5 は解決、W2/W6/W8 は判断材料が出揃った状態**まで進んだ。
残る未決は実質3件で、いずれも**技術的事実ではなく製品としての意思**を要する:
W2(テストのシグナルを消してよいか)/ W6(display-only の主張を弱めてよいか)/
W8(通知の攻撃性を上げてよいか)。改善は上の優先度と着手条件に従い、
