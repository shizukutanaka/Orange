...
### A. 4件の「既定として出荷済み」の設計判断 — 穴ではなく**選択**

**この4件はプロダクトの欠落ではない。** どれも既に**具体的な挙動として出荷されている**
(実測で確認: W6=末尾4桁 allow / W8=フルスクリーン intent 不使用 / §1-12=0990 層なし /
§1-13=oracle 維持・pre-push で実行)。「未決定」という状態は存在せず、
**既定が選ばれていて、それが安全側**というだけである。長くこれを「未解決の4件」と
書いてきたのは記述の誤りで、読む者(過去の自分を含む)に未完成の印象を与えていた。

したがって **プロダクトは出荷可能な完成状態**であり、以下の表は
「もし方針を変えるなら」の分岐点であって、リリースの前提条件ではない。

| # | 既定(出荷中の挙動) | なぜ既定が安全側か | 変えるなら問うべきこと |
|---|---|---|---|
| W6 / §2-2 | 末尾4桁での allow | 完全番号を保存しない = 「display-only」の主張を最も強く保てる。誤許容は1万分の1で、しかも16層を通過する必要がある | 「display-only」の主張を弱めてよいか |
| W8 / §1-4 | 全画面割り込みは**しない**(DND突破・振動は ON、音は意図的に OFF) | 誤警告時に高齢者の画面を奪わない。鳴動中に音を足すと文言を読む一瞬を潰す | 割り込みの強さを上げてよいか |
| §1-12 | 0990 に専用層を**設けない** | 0990 の課金は発信側。着信を黙らせる価値が低く、層は誤ブロック面積を増やす | 発信警告として作るか(着信層ではなく) |
| §1-13 | `oracle_decision.py` を**維持** | 削除は情報の消失。維持コストは pre-push の数秒のみ | Python 規則の重複を保つ価値があるか |

いずれも情報は出揃っており、**変更する場合にのみ**人間の価値判断が要る。
変更しない限り、上の既定のまま出荷して問題はない。
...
いずれも**選択肢と根拠は記録済み**。上表のとおり既定は出荷済みなので、
足りないのは「決定」ではなく、**方針を変える意思があるかどうか**だけである。

### B. 人間の「権限」が要る3手順 — 手順は `RELEASING.md` に順序付きで完備

~~main マージ~~(**完了**: PR #4 / マージコミット `b513490`)→ CI 有効化 → Pages 有効化
→ タグ+Release+APK(計約3分、**判断は不要**)。
...
B の残り3手順が済んだ時点で、この節ごと削除してよい(A は既定が出荷済みなので
リリースを妨げない)。
