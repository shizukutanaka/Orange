...
| # | 弱点 | 状態 |
|---|------|------|
| W1 | テストが CI で一度も実行されていなかった(静的ゲートは @Test を数えるだけ)| `tools/run-pure-tests.sh` で純粋層199テストは実行可能に。ストア層(~150, FakePrefs 依存)は未実行のまま |
...
### 今すぐ着手可(判断不要・機械的)
1. **ストア層テストの実行拡張**: `/tmp` 限定の最小 Android 型スタブ(SharedPreferences/Base64/keystore 型。ロジックなし、Base64 のみ `java.util.Base64` 委譲)で SpamCache/OutboundGuard/WangiriTracker/RepeatCaller/RateLimiter/AllowSuffix/BlockHistory のテスト(~150)を初実行し、失敗を「実バグ/陳腐化/設計判断」に分類。スタブはリポジトリにコミットしない。
...
3. 検証: Gradle ビルドはサンドボックスで不可 → `bash tools/run-pure-tests.sh`(純粋層199テスト、SDK不要)+ 机上トレース。push 後の CI は当てにしない(W1)。
...
bash tools/run-pure-tests.sh          # 期待値: 199 run / 2 failures(W2 の意図的シグナル)
