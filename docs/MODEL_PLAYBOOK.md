...
4. W6: History にハッシュを持たせ Allow を exact 化するか(= 「display-only」の主張を弱めてよいか)。中間案(末尾4桁+桁数)は §2-2 に記載済み。
5. W8: 警告のフルスクリーンインテント化(= 通知の攻撃性を上げてよいか)。CHI 2025 が支持、誤警告時の負荷が反対根拠。**必ず選択肢を提示して人間が決める**。
...
W6(display-only の主張を弱めてよいか)/ W8(通知の攻撃性を上げてよいか)。改善は上の優先度と着手条件に従い、
