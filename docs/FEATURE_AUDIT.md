...
### 1-13. `oracle_decision.py` は自称する「振る舞いの tripwire」ではない【要判断: 維持か削除か】
- **発見経緯(2026-07)**: 全ゲートが green の状態で「**ゲート自身は正しいか**」をソクラテス式に問い、
  実験で確かめた。`DomesticSpoofDetector` の連続同一数字閾値を 8→9 に変える(実際の振る舞い回帰)と:

  | ゲート | 結果 |
  |---|---|
  | `tools/run-pure-tests.sh` | **exit 1** — 失敗2件をテスト名付きで報告 ✅ |
  | `tools/oracle_decision.py` | **exit 0** —「All 31 oracle cases passed」❌ |

- **原因**: oracle は Kotlin の規則を **Python で再実装**し、**自前の表**と照合するだけ。
  Kotlin を一切読まないので、Kotlin 側の変更は原理的に見えない。
  旧ヘッダの「両者が食い違えばどちらかが誤り — 出荷前に調査せよ」は正しいが、
  **食い違いを検出する主体が存在しない**ため、誰も調査を促されない。
  (なお最初の2回の実験は失敗した — `0120` を10桁必須リストから外す変更は
  「地理番号は11桁不可」規則に**マスクされて**挙動が変わらず、`length < 9` への変更は
  JP番号が10-11桁なので**発火しなかった**。規則の冗長性そのものは健全である証拠。)
- **前提の失効**: 存在理由は「JVM の無い環境での高速反復」だったが、
  `run-pure-tests.sh` が **SDK もネットワークも無しで実 Kotlin 435テストを約30秒**で回すため、
  この script が動く環境ならほぼ確実にエンジン本体を直接検証できる。
- **実施済み(機械的修正)**: docstring を実測結果ごと書き換え、成功メッセージを
  「Python rules self-check only — engine behaviour is verified by run-pure-tests.sh」に変更。
  **偽の安心感が最大の害**だったため、そこを断った。
- **要判断(削除か維持か)**: 手で同期する重複実装をコストとして払い続けるか。
  - **維持論**: 実行コストほぼゼロ。両方を読む人には差異が見える(differential testing の弱い形)。
  - **削除論**: 検出器の無い手動同期は、今セッションで自動化した数値ドリフトと**同じ欠陥クラス**。
...
### 1-3. ディレクトリ陳腐化への構造的対策【✅ 2026-07 制約として受容(打てる手は実施済み)】
...
### 1-4. 警告通知の実効性が未検証【✅ 文言は解決済み / 残るはフルスクリーン化の是非のみ】
...
### 1-5. 信頼済みインサイダー脅威モデルの欠落【✅ 2026-07 文書化により解決(PIN は意図的に不採用)】
...
### 1-6. テストスイートがCIで一度も実行されていなかった【✅ 2026-07 解決済み(CI 作成 + 435テスト実行基盤)】
...
### 2-2. 解除メカニズムの二重化(Allow vs Restore)【要判断: 現状維持か完全解か】
