...
**(5) 安価な中間案(未実装)**: ハッシュを持たずとも、
**末尾4桁 + 桁数**(10桁固定電話 vs 11桁携帯)を保存すれば衝突空間を数倍に絞れる。
プライバシー露出はゼロ(桁数はマスク済み表示から既に推測可能)。
完全な解決ではないが、`display-only` を一切損なわずに実害を減らせる。

**判断が要る点**: (3) の「display-only を弱めてよいか」が本質。
